...
public sealed class AccountController : Controller
{
    ...
    public IActionResult Registration() => View("Register", new RegisterMemberViewModel());
    ...
    public async Task<IActionResult> Registration(RegisterMemberViewModel model)
    {
        ...
        try
        {
            ...
            await _memberSignInManager.PasswordSignInAsync(identityUser.UserName ?? model.Username, model.Password, isPersistent: true, lockoutOnFailure: true);
        }
        catch (Exception ex)
        {
            TryDeleteMember(member);
    ...
    public async Task<IActionResult> LogIn(LoginMemberViewModel model)
    {
        if (!ModelState.IsValid)
        {
            ...
            return Redirect("/members/log-in");
        }

        var signInResult = await _memberSignInManager.PasswordSignInAsync(model.Username, model.Password, isPersistent: true, lockoutOnFailure: true);
